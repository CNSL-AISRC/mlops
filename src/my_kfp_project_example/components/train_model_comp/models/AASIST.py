...
class GraphAttentionLayer(nn.Module):
    ...
    def _derive_att_map(self, x: Tensor):
        '''
        x           :(#bs, #node, #dim)
...
class HtrgGraphAttentionLayer(nn.Module):
    ...
    def _derive_att_map(self, x: Tensor, num_type1: int, num_type2: int):
        '''
        x           :(#bs, #node, #dim)
        out_shape   :(#bs, #node, #node, 1)
        '''
...
class GraphPool(nn.Module):
    ...
    def top_k_graph(self, scores: Tensor, h: Tensor, k: float):
        """
        args
        =====
...
class Residual_block(nn.Module):
    def __init__(self, nb_filts, first=False):
        ...
        if nb_filts[0] != nb_filts[1]:
            ...
            self.conv_downsample = nn.Identity() # for tracing
