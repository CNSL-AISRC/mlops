...
def train_model(processed_data: str, config: str) -> str:
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment.experiment_id) as run:
        ...
        mlflow.pytorch.log_model(pytorch_model=model, artifact_path="pytorch_last_model", registered_model_name=os.getenv("MLFLOW_MODEL_NAME"), signature=signature)
        
        # convert to scripted model and log the model
        scripted_pytorch_model = torch.jit.script(model)
        mlflow.pytorch.log_model(pytorch_model=scripted_pytorch_model, artifact_path="pytorch_last_model_scripted", registered_model_name=os.getenv("MLFLOW_MODEL_NAME"), signature=signature)
        
        
        model_uri = f"{run.info.artifact_uri}/pytorch_last_model_scripted"
        print("Trained model:", model_uri)
        return model_uri
    #return "s3://mlflow/3/43570b93dced4576ac91cc1e34fa5a76/artifacts/last_model" # DUMMY OUTPUT FOR DEBBUGING
